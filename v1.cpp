...
void CGame::checkMove(char move, Maze& a_maze , Creature& a_creature){
	...
	if(move=='w')
    {
        ...
        {
			a_creature.addPoints(a_maze.getReward(a_maze,a_row-1,a_col));
			a_maze.updateMaze(a_creature,a_row-1,a_col);
			a_creature.updateCreaturePosition(a_row-1,a_col);
		}
    }   
    else if (move=='a'){
		if(a_maze.isValidMove(a_row,a_col-1)==true){
			a_creature.addPoints(a_maze.getReward(a_maze,a_row,a_col-1));
			a_maze.updateMaze(a_creature,a_row,a_col-1);
			a_creature.updateCreaturePosition(a_row,a_col-1);
	    }
    }
        else if(move=='d'){
            if(a_maze.isValidMove(a_row,a_col+1)==true){
